...
def test_sample_video(tmpdir: py.path.local, monkeypatch):
    ...
    sample_video.sample_video(video_dir, str(sample_dir), rerun=True)
...
def test_sample_video_with_start_time(tmpdir: py.path.local, monkeypatch):
    monkeypatch.setattr(ffmpeg, "extract_frames", mock_extract_frames)
    monkeypatch.setattr(ffmpeg, "probe_video_streams", mock_probe_video_streams)
    root = os.path.join(_PWD, "data/mock_sample_video")
    video_dir = os.path.join(root, "videos")
    ...
    sample_video.sample_video(
        video_dir, str(sample_dir), video_start_time=video_start_time_str, rerun=True
    )
    samples = sample_dir.join("hello.mp4").listdir()
    assert len(samples), "expect samples but got none"
    for idx, sample in enumerate(sorted(samples)):
