...
- Process and upload all the images in directory `path/to/images` and its sub-directories for user `mapillary_user`, while skipping duplicate images. Here duplicate images are specified as consecutive images that are less 0.5 meter apart according to image `GPS` and has less than 1° camera angle difference according to image direction.
