...
def process_complete(log_root, completed):
    process_complete_path = os.path.join(log_root, "process_")
    if completed:
...
def insert_MAPJson(full_image_list, import_path, master_upload, verbose, skip_insert_MAPJson):
    ...
    for image in full_image_list:
        ...
        sequence_properties_process_success_path = os.path.join(
            log_root, "sequence_process_success")
        upload_params_properties_process_success_path = os.path.join(
            log_root, "upload_params_process_success")
        # optional
        import_meta_data_properties_process_success_path = os.path.join(
            log_root, "import_meta_data_process_success")
        ...
        user_properties_process_json = os.path.join(
            log_root, "user_process.json")
        import_meta_data_process_json = os.path.join(
            log_root, "import_meta_data_process.json")
        geotag_properties_process_json = os.path.join(
            log_root, "geotag_process.json")
        sequence_properties_process_json = os.path.join(
            log_root, "sequence_process.json")
        upload_params_properties_process_json = os.path.join(
            log_root, "upload_params_process.json")
        settings_upload_hash_process_json = os.path.join(
            log_root, "settings_upload_hash.json")
        ...
        # user properties
        final_mapillary_image_description = update_mapillary_description(
            final_mapillary_image_description, user_properties_process_json, verbose)
        # import properties
        ...
        if (os.path.isfile(import_meta_data_process_json) and not os.path.isfile(import_meta_data_properties_process_success_path)) or (not os.path.isfile(import_meta_data_process_json) and os.path.isfile(import_meta_data_properties_process_success_path)):
            ...
            final_mapillary_image_description = update_mapillary_description(
                final_mapillary_image_description, import_meta_data_process_json, verbose)

        # geotag properties
        final_mapillary_image_description = update_mapillary_description(
            final_mapillary_image_description, geotag_properties_process_json, verbose)

        # sequence properties
        final_mapillary_image_description = update_mapillary_description(
            final_mapillary_image_description, sequence_properties_process_json, verbose)

        # sequence properties
        if not master_upload:
            final_mapillary_image_description = update_mapillary_description(
                final_mapillary_image_description, settings_upload_hash_process_json, verbose)
        ...
        if not skip_insert_MAPJson:
            ...
            process_complete(log_root, True)
        else:
            process_complete(log_root, False)
