...
from . import api_v4, config, constants, description, exceptions, types
...
def authenticate(
    ...
    if delete:
        ...
        if jwt:
            user_items: description.UserItem = {"user_upload_token": jwt}
...
def _validate_profile(user_items: description.UserItem) -> description.UserItem:
    try:
        jsonschema.validate(user_items, description.UserItemSchema)
...
def _verify_user_auth(user_items: description.UserItem) -> description.UserItem:
    """
...
def _list_all_profiles(profiles: dict[str, description.UserItem]) -> None:
