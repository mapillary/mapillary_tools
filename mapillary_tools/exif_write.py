...
class ExifEdit:
    ...
    def decimal_to_dms(value: float) -> \
        ...
        # Convert decimal position to Exif degrees, minutes, seconds rationals

        deg: int = int(value)
        min: int = int(value := (value - deg) * 60)
        sec: float = (value - min) * 60

        return (deg, 1), (min, 1), (Fraction.from_float(sec)
                                    .limit_denominator()
                                    .as_integer_ratio())
    ...
    def add_gps_datetime(self, dt: datetime.datetime) -> None:
        ...
        self._ef["GPS"][piexif.GPSIFD.GPSTimeStamp] = (
            (dt.hour, 1),
            (dt.minute, 1),
            (Fraction.from_float(dt.second + dt.microsecond)
             .limit_denominator()
             .as_integer_ratio()
            )
        )
